class Item {
  ...
  getCircle() {
    const {
      ...
      const matches = getComputedStyle(nodeScale).transform.match(matrixRegex);
      ...
      const { x, y } = getCenter(nodeTranslate);
      const size = scale * getItemSize();
  ...
  destroy(hadCollision) {
    ...
    const { nodeTranslate, nodeScale } = this;
...
class Human extends Item {
  constructor() {
    const color = humanColors[r(humanColors.length) - 1];
...
class Score {
  constructor(x, y, value) {
      ...
      const delta = ifSmallScreen(18, 35);
      const targetX = x - delta;
